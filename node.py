...
class Node:
    ...
    #TODO
    @staticmethod
    def gini(left_pos, left_neg, right_pos, right_neg):
        left = left_pos + left_neg
        right = right_pos + right_neg
        total = left + right

        gini_left = 1 - (left_pos / left) ** 2 - (left_neg / left) ** 2 if left > 0 else 0
        gini_right = 1 - (right_pos / right) ** 2 - (right_neg / right) ** 2 if right > 0 else 0
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left_pos = np.count_nonzero(y[:idx + 1] == 1)
            left_neg = (idx + 1) - left_pos  # ilość elementów do indeksu idx minus ilość pozytywnych daje ilość negatywnych
            right_pos = np.count_nonzero(y[idx + 1:] == 1)
            right_neg = len(y) - (idx + 1) - right_pos  # całkowita ilość elementów minus ilość elementów do indeksu idx minus ilość pozytywnych daje ilość negatywnych

            gain = Node.gini(left_pos, left_neg, right_pos, right_neg)
    ...
    def gini_best_score(self, y, possible_splits):
        ...
        for idx in possible_splits:
            left_pos = np.count_nonzero(y[:idx + 1] == 1)
            left_neg = (idx + 1) - left_pos  # liczba elementów do indeksu idx minus liczba pozytywnych
            right_pos = np.count_nonzero(y[idx + 1:] == 1)
            right_neg = len(y) - (idx + 1) - right_pos  # całkowita liczba elementów minus elementy do indeksu idx minus liczba pozytywnych
